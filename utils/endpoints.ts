...
const urlSwitch = (key: string, date: string | undefined, lon: number | undefined, lat: number | undefined): string => {    
    switch(key){
        case "apod": 
...
export const fetchedData = async (key: string, date = new Date(), lon ?:number | undefined, lat ?:number | undefined): Promise<any> => {
    ...
    if (thisDate != undefined){
        ...
        [newDay, newMonth ] = (key != 'epic') ? [day.toString().padStart(2, '0'), month.toString().padStart(2, '0')] : [day, month];
        newDate = [year, newMonth, newDay].join('-');
    }

    let url = urlSwitch(key, newDate, lon, lat)
    let fetch = await fetcher(url);
    return fetch;
  };