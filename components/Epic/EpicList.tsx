...
export default function EpicList({ data }: Props) {
  ...
  return (
    <div className="flex w-5/12 h-5/12 p-1 rounded-sm shadow-2xl items-center justify-center bg-purple-900  sm:w-3/6 md:w-4/6 lg:w-5/6">
      {slidesCount > 0 ? (
        <div className="flex w-full relative overflow-hidden">
          ...
          <p
            className={"select-none absolute left-0" + { arrowStyles }}
            onClick={prevSlide}
          >
            &#10094;
          </p>
          <p
            className={"select-none absolute right-0 " + { arrowStyles }}
            onClick={nextSlide}
          >
            &#10095;
          </p>
        ...
        <div className="w-full h-96 flex flex-col items-center justify-center">
          <h1 className="mt-4 mb-1 text-lg">No Images Found</h1>
          <p className="max-w-md">No images taken on this date, try another!</p>
        </div>
      )}
    </div>
  );
